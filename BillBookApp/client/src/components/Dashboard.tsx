...
import { Users, Package, MessageCircle, Plus, FileText, TrendingUp, ArrowDownLeft, Calculator, CheckCircle, ChevronRight, Eye, MoreVertical, Download, ChevronDown, Building2, Check } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({
    ...
    return (
        <div className="pb-4 md:pb-6 bg-background min-h-screen font-sans" >
            {/* Google Static Top Bar / Header */}
            <div className="pt-safe pb-2 px-6 sticky top-0 z-30 bg-background/95 backdrop-blur-md" >
                <div className="max-w-5xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <button
                            onClick={onOpenSidebar}
                            className="p-2 -ml-2 rounded-full hover:bg-surface-container-high transition-colors md:hidden"
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6 text-foreground"><line x1="4" x2="20" y1="12" y2="12" /><line x1="4" x2="20" y1="6" y2="6" /><line x1="4" x2="20" y1="18" y2="18" /></svg>
                        </button>
                        <div className="flex flex-col">
                            ...
                            <div className="relative">
                                ...
                                >
                                    <h1 className="text-3xl md:text-4xl font-bold font-heading text-foreground tracking-tight group-hover:text-google-blue transition-colors">
                                        {company?.name || 'My Business'}
                                    </h1>
                                    <ChevronDown className={`w-6 h-6 text-muted-foreground transition-transform duration-300 ${showCompanyDropdown ? 'rotate-180' : ''}`} />
                                </button>
                                    ...
                                    {showCompanyDropdown && (
                                        <>
                                            <motion.div
                                                ...
                                                exit={{ opacity: 0, y: -10 }}
                                                className="absolute top-full left-0 mt-4 w-72 bg-surface-container rounded-[28px] shadow-google-lg border border-border overflow-hidden z-[100]"
                                            >
                                                <div className="p-4 border-b border-border bg-surface-container-low">
                                                    <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest px-2 mb-2">Switch Business</p>
                                                    <div className="space-y-1">
                                                        {companies.map((comp) => (
                                                            <button
                                                                ...
                                                                className={`w-full flex items-center gap-3 p-3 rounded-2xl transition-all ${comp.id === company?.id
                                                                    ? 'bg-google-blue text-white shadow-google'
                                                                    : 'hover:bg-surface-container-highest text-foreground'
                                                                    }`}
                                                            >
                                                                <Building2 className="w-5 h-5 flex-shrink-0" />
                                                                <span className="flex-1 text-left font-bold text-sm truncate">{comp.name}</span>
                                                                {comp.id === company?.id && <Check className="w-4 h-4" />}
                                                            </button>
                                                        ))}
                                                    </div>
                                                </div>
                                                <button
                                                    ...
                                                    className="w-full p-4 flex items-center justify-center gap-2 bg-google-blue/10 text-google-blue font-bold text-sm hover:bg-google-blue hover:text-white transition-all"
                                                >
            ...
            <motion.div
                ...
                initial="hidden"
                animate="show"
                className="px-4 md:px-8 max-w-5xl mx-auto space-y-4"
            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <motion.div
                        ...
                        className="relative overflow-hidden bg-white dark:bg-slate-900 border border-white/50 dark:border-slate-800 p-7 rounded-[32px] shadow-xl shadow-slate-200/50 dark:shadow-none group cursor-pointer active:scale-[0.98] transition-all"
                    >
                        {/* Decorative Gradient Background similar to Auth */}
                        <div className="absolute inset-0 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-slate-800 dark:to-slate-900 opacity-50" />

                        {/* Abstract Shapes */}
                        <div className="absolute top-[-20%] right-[-10%] w-32 h-32 rounded-full bg-blue-200/20 blur-xl" />
                        <div className="absolute bottom-[-10%] left-[-10%] w-24 h-24 rounded-full bg-indigo-200/20 blur-xl" />

                        <div className="relative z-10">
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    <p className="text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-1">Total Outstanding</p>
                                    <h2 className="text-4xl md:text-5xl font-black font-heading text-slate-900 dark:text-white tracking-tight">
                            ...
                            <div className="flex items-center gap-2 text-xs font-bold bg-white/50 dark:bg-slate-800/50 w-fit px-3 py-1.5 rounded-full border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 uppercase tracking-wider backdrop-blur-sm">
                ...
                {showCreateDialog && (
                    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-md p-6">
                        <motion.div
                            ...
                            className="bg-surface-container rounded-[32px] w-full max-w-sm shadow-google-lg overflow-hidden border border-white/20"
                        >
                            <div className="p-8 pb-4">
                                <h2 className="text-2xl font-black text-foreground mb-1">New Business</h2>
                                <p className="text-xs text-muted-foreground font-bold uppercase tracking-widest mb-8">Setup a new entity</p>

                                <div className="space-y-6">
                                    <div>
                                        <label className="block text-[10px] font-black text-google-blue uppercase tracking-widest mb-2 ml-1">Business Name</label>
                                        <input
                                            type="text"
                                            value={newCompanyName}
                                            onChange={(e) => setNewCompanyName(e.target.value)}
                                            placeholder="e.g. Acme Corp"
                                            className="w-full px-5 py-4 bg-surface-container-highest rounded-2xl border border-border outline-none focus:border-google-blue text-foreground font-bold text-lg"
                                            autoFocus
                                        />
                                    </div>
                                    <p className="text-[10px] text-muted-foreground leading-relaxed px-1">
                                        Tip: You can customize address, GST and other settings later for each business individually.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="p-8 flex gap-3">
                                <button
                                    ...
                                    className="flex-1 py-4 bg-surface-container-low text-muted-foreground rounded-2xl font-bold hover:bg-surface-container-high transition-colors"
                                    disabled={creating}
                                >
                                    Cancel
                                </button>
                                <button
                                    ...
                                    className="flex-1 py-4 bg-google-blue text-white rounded-2xl font-bold shadow-google hover:scale-105 active:scale-95 transition-all disabled:opacity-50"
                                >
