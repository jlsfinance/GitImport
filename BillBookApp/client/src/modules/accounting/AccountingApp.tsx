...
import SmartCalculator from './pages/SmartCalculator'; // Added // Added
...
const AccountingApp: React.FC = () => {
    ...
    const handleQuickShare = (invoice: Invoice) => {
        ...
        if (customer?.phone) {
            ...
            setPendingShareInvoice(invoice);
    ...
    return (
        ...
        >
          ...
          ${isSidebarOpen ? 'flex' : (['VIEW_INVOICE', 'CREATE_INVOICE', 'EDIT_INVOICE', 'PUBLIC_VIEW_INVOICE', 'CREATE_PURCHASE', 'EDIT_PURCHASE', 'SMART_CALCULATOR'].includes(currentView) ? 'hidden' : 'hidden md:flex')}
        `}
            >
                <div
            ...
            <main className="flex-1 overflow-y-auto h-full relative w-full scroll-smooth bg-slate-50 dark:bg-slate-900 pb-20 md:pb-0">
                {!['DASHBOARD', 'VIEW_INVOICE', 'CREATE_INVOICE', 'EDIT_INVOICE', 'PUBLIC_VIEW_INVOICE', 'CREATE_PURCHASE', 'EDIT_PURCHASE', 'SMART_CALCULATOR'].includes(currentView) && (
                    <div className="md:hidden sticky top-0 z-30 bg-white/95 dark:bg-slate-950/95 backdrop-blur-sm border-b border-slate-200/50 dark:border-slate-800/50 shadow-sm pt-safe">
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        ...
                        {(currentView === ViewState.ALL_INVOICES || currentView === ViewState.INVOICES) && <AllInvoices title="Sales" createLabel="Add Sale" onView={handleViewInvoice} onEdit={handleEditInvoice} onDelete={handleDeleteInvoice} />}
                        {currentView === ViewState.PURCHASES && <AllInvoices title="Purchases" createLabel="Add Purchase" invoices={purchases} onView={handleViewInvoice} onEdit={handleEditPurchase} onDelete={handleDeletePurchase} />}
            ...
            {!['VIEW_INVOICE', 'CREATE_INVOICE', 'EDIT_INVOICE', 'PUBLIC_VIEW_INVOICE', 'CREATE_PURCHASE', 'EDIT_PURCHASE', 'SMART_CALCULATOR'].includes(currentView) && (
                <MobileBottomNav
                    currentView={currentView}
                    onChangeView={(view) => {
                        if (view === ViewState.IMPORT) {
