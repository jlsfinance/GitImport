...
const Sidebar: React.FC = () => {
    ...
    const { signOut } = useRecordAuth();

    const [expandedMenus, setExpandedMenus] = useState<string[]>(['Ledger Management', 'Accounts']);
    ...
    const menuItems: MenuItem[] = [
        { title: 'Dashboard', path: '/records', icon: 'dashboard' },
        {
            title: 'Ledger Management',
            icon: 'account_balance',
            submenu: [
                { title: 'All Entries', path: '/records/all', icon: 'list_alt' },
                { title: 'New Entry', path: '/records/new', icon: 'add_circle' }, // fka /records/new (AddRecord)
                { title: 'Calculator', path: '/records/tools/calculator', icon: 'calculate' },
            ]
        },
        {
            title: 'Accounts',
            icon: 'payments',
            submenu: [
                { title: 'Overview', path: '/records/finance', icon: 'finance' },
                { title: 'Receipts', path: '/records/receipts', icon: 'receipt_long' },
                { title: 'Approvals', path: '/records/approvals', icon: 'verified' },
                { title: 'Payment Out', path: '/records/new-entry', icon: 'monetization_on' }, // fka /disbursal
                { title: 'Collection List', path: '/records/due-list', icon: 'pending_actions' },
            ]
        },
        { title: 'Customers', path: '/records/customers', icon: 'group' },
        { title: 'Partners', path: '/records/partners', icon: 'handshake' },
        { title: 'Tools', path: '/records/tools', icon: 'construction' },
        { title: 'Downloads', path: '/records/downloads', icon: 'folder_open' },
        { title: 'Settings', path: '/records/settings', icon: 'settings' },
    ];
            ...
            >
                ...
                <div className="p-6 border-t border-border bg-surface-container-low flex-shrink-0">
                    ...
                    <button
                        onClick={() => {
                            closeSidebar();
                            navigate('/records/company-selector');
                        }}
                        className="w-full mb-2 flex items-center gap-4 px-6 py-4 rounded-[24px] bg-surface-container text-muted-foreground hover:bg-surface-container-high hover:text-foreground transition-all font-bold text-sm group cursor-pointer"
                    >
