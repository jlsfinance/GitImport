...
const BottomNav: React.FC = () => {
  ...
  const menuItems = [
    { path: '/records/tools', label: 'Tools', icon: 'construction', color: 'text-orange-500', bgColor: 'bg-orange-50' },
    { path: '/records/reports', label: 'Reports', icon: 'bar_chart', color: 'text-purple-500', bgColor: 'bg-purple-50' },
    { path: '/records/notifications', label: 'Notifications', icon: 'notifications', color: 'text-blue-500', bgColor: 'bg-blue-50' },
    { path: '/records/settings', label: 'Settings', icon: 'settings', color: 'text-gray-500', bgColor: 'bg-gray-50' },
  ];
    ...
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-surface-container/95 dark:bg-surface-container-low/95 backdrop-blur-md border-t border-border/50 pb-safe z-30 min-h-[80px] lg:hidden">
        <div className="flex justify-around items-center h-full px-4 max-w-lg mx-auto py-2">
          <NavButton
            isActive={activePath === '/records'}
            onClick={() => handleNavClick('/records')}
            icon="dashboard"
            label="Home"
          />
          <NavButton
            isActive={activePath.includes('/records/customers')}
            onClick={() => handleNavClick('/records/customers')}
            icon="group"
            label="Clients"
          />
          <NavButton
            isActive={activePath.includes('/records/all') || (activePath.includes('/records/view'))}
            onClick={() => handleNavClick('/records/all')}
            icon="account_balance"
            label="Records"
          />
          <NavButton
            isActive={activePath.includes('/records/finance')}
            onClick={() => handleNavClick('/records/finance')}
