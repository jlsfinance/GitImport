...
import { fetchCustomerById, fetchRecordsByCustomerId, deleteCustomer } from '../services/dataService';
...
const CustomerProfile: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (customerData) {
        const recordsData = await fetchRecordsByCustomerId(id);
    ...
    >
      ...
      <div className="sticky top-0 z-50 flex items-center justify-between bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-md p-4 border-b border-slate-200 dark:border-slate-800 print:hidden">
        ...
        <div className="flex gap-2">
          <Link to={`/records/customers/edit/${id}`} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                    ...
                    <div key={record.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white dark:bg-[#1e2736] border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow">
                      <div className="space-y-1 mb-4 sm:mb-0">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400">
                          Created on: {new Date(record.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
                        </p>
                      </div>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-8 gap-y-2 text-sm w-full sm:w-auto">
                        ...
                        <div>
                          ...
                          <span className="font-bold text-slate-900 dark:text-white">{formatCurrency(record.installmentAmount)}</span>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <span className="font-bold text-slate-900 dark:text-white">{record.rate}%</span>
                        </div>
                      </div>
                    </div>
                  ))}
