...
const safeFormatDate = (dateString: string | undefined | null, formatStr: string = 'dd-MMM-yyyy') => {
    ...
    try {
        const date = (dateString as any)?.toDate ? (dateString as any).toDate() : new Date(dateString as string);
        if (isValid(date)) {
...
const AllRecords: React.FC = () => {
    ...
    const fetchRecords = useCallback(async () => {
        ...
        try {
             ...
             * 'records' = current collection name (neutral)
             * Legacy collection queried for backward compatibility only.
             * All NEW records are saved to 'records' collection.
             */
            const [recordsSnap, legacySnap] = await Promise.all([
                getDocs(query(collection(db, "records"), where("companyId", "==", currentCompany.id))),
                // LEGACY: Query old collection for migration (read-only, will be deprecated)
                getDocs(query(collection(db, "loans"), where("companyId", "==", currentCompany.id)))
            ]);

            const mergedDocs = [...recordsSnap.docs, ...legacySnap.docs];
            ...
            recordsData.sort((a: any, b: any) => {
                const dateA = a.date?.toDate ? a.date.toDate() : (new Date(a.date) || new Date(0));
                const dateB = b.date?.toDate ? b.date.toDate() : (new Date(b.date) || new Date(0));
    ...
    const handleDeleteRecord = async () => {
        ...
        try {
            ...
            showErrorAlert('DATA_DELETE_FAILED', error);
        } finally {
            setDeletingId(null);
    ...
    const generateRecordCard = async (record: RecordItem) => {
        ...
        try {
            // Only amount and tenure are strictly required for PDF generation
            if (!record.amount || !record.tenure) {
                throw new Error("Incomplete record details (missing amount or tenure).");
            }
            ...
            // Schedule
            const head = [["No", "Outstanding Date", "Amount", "Principal", "Service Fee", "Balance"]];
            const body: any[] = [];

            if (record.repaymentSchedule && record.repaymentSchedule.length > 0) {
                record.repaymentSchedule.forEach((inst: any, index: number) => {
                    body.push([
                        inst.installmentNumber || inst.instNumber || (index + 1),
                        inst.dueDate ? format(new Date(inst.dueDate), 'dd-MMM-yy') : 'N/A',
                ...
                // Legacy Fallback - generate schedule from record data
                let balance = record.amount;
                const monthlyMarkupRate = (record.rate || (record as any).markupRate || (record as any).interestRate || 0) / 12 / 100;

                for (let i = 1; i <= record.tenure; i++) {
                    // Fallback: calculate installment if not available
                    const instAmount = record.installmentAmount || (record as any).installment || (record as any).emi || Math.ceil(record.amount / record.tenure);
                    const feePayment = balance * monthlyMarkupRate;
                    const basePayment = instAmount - feePayment;
                    balance -= basePayment;
    ...
    const StatusBadge = ({ status }: { status: string }) => {
        let classes = "inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-widest ring-1 ring-inset ";
        if (status === 'Approved') classes += "bg-emerald-50 text-emerald-700 ring-emerald-600/20 dark:bg-emerald-950/30 dark:text-emerald-400";
        else if (status === 'Active' || status === 'Settled') classes += "bg-indigo-50 text-indigo-700 ring-indigo-600/20 dark:bg-indigo-900/30 dark:text-indigo-400";
        else if (status === 'Rejected') classes += "bg-rose-50 text-rose-700 ring-rose-600/20 dark:bg-rose-950/30 dark:text-rose-400";
        else classes += "bg-slate-50 text-slate-500 ring-slate-600/10 dark:bg-slate-800 dark:text-slate-400";

        return <span className={classes}>{status === 'Given' ? 'Active' : status}</span>;
    };
    ...
    return (
        ...
        >
            ...
            <div className="sticky top-0 z-20 px-4 pt-4 pb-4 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-md border-b border-slate-100 dark:border-slate-800" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 16px)' }}>
                <div className="flex items-center justify-between mb-4">
                    <h1 className="text-2xl font-black tracking-tight">Records</h1>
                    <Link to="/records/new" className="h-10 w-10 rounded-full bg-primary text-white shadow-lg shadow-primary/30 flex items-center justify-center hover:brightness-110 active:scale-95 transition-all">
                        <span className="material-symbols-outlined font-variation-FILL">add</span>
                    </Link>
                </div>
                <div className="relative group">
                    ...
                    <input
                        ...
                        className="w-full h-11 pl-10 pr-4 rounded-xl border-none bg-slate-100 dark:bg-slate-800 shadow-inner focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                        value={searchTerm}
            ...
            {/* List */}
            <main className="flex flex-col gap-3 px-3 pt-3 sm:px-4 sm:pt-4">
                {loading ? (
                    ...
                    filteredRecords.map((record) => (
                        <div key={record.id}>
                            <div
                                ...
                                className="glass-card relative rounded-2xl p-4 sm:p-5 hover:bg-white/90 dark:hover:bg-slate-800/90 hover:shadow-xl hover:shadow-indigo-500/10 cursor-pointer transition-all duration-300 group border border-white/40 dark:border-slate-700/40"
                            >
                                ...
                                <div className="flex justify-between items-center gap-3">
                                    <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                                        <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-2xl bg-gradient-to-br from-indigo-500/10 to-blue-500/10 flex items-center justify-center text-indigo-600 font-black text-base sm:text-lg border border-indigo-500/10 shrink-0">
                                            {record.customerName?.charAt(0).toUpperCase()}
                                        </div>
                                        <div className="space-y-0.5 min-w-0 flex-1">
                                            <h3 className="font-black text-sm sm:text-base truncate text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors capitalize leading-tight">{record.customerName.toLowerCase()}</h3>
                                            <div className="flex flex-wrap items-center gap-2">
                                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest hidden sm:inline">#{record.id?.slice(0, 8)}</span>
                                                <StatusBadge status={record.status} />
                                            </div>
                                        </div>
                                    </div>
                                    <div className="text-right flex flex-col items-end gap-0.5 sm:gap-1 flex-shrink-0">
                                        <p className="text-base sm:text-lg font-black text-slate-900 dark:text-white tracking-tighter">{formatCurrency(record.amount)}</p>
                                        <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">
                                            {(() => {
                                                const d: any = record.date;
                                                const dateObj = d?.toDate ? d.toDate() : (d ? new Date(d) : null);
                                                return dateObj && !isNaN(dateObj.getTime()) ? format(dateObj, 'dd MMM, yy') : 'N/A';
                                            })()}
                                        </p>
                                    </div>
                                    <button
                                        ...
                                        className="sm:ml-2 p-1.5 sm:p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 transition-colors -mr-2 sm:mr-0"
                                    >
                ...
                activeMenuId && (() => {
                    ...
                    return (
                        <>
                            ...
                            ></div>
                            <div className="fixed inset-x-0 bottom-0 z-50 transform transition-transform duration-300 ease-out md:inset-auto md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:w-full md:max-w-md md:transition-all">
                                <div className="bg-white dark:bg-[#1e2736] rounded-t-[2.5rem] md:rounded-2xl shadow-[0_-20px_50px_rgba(0,0,0,0.2)] md:shadow-2xl overflow-hidden pb-safe">
                                    {/* Handle - Mobile Only */}
