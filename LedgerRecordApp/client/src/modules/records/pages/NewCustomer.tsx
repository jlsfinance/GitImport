...
const NewCustomer: React.FC = () => {
    ...
    const uploadPhoto = async (file: File): Promise<string> => {
        try {
            // Use centralized ImageUploadService (Cloudinary + ImgBB fallback)
            const url = await ImageUploadService.uploadCompressedImage(file, 'customers');
            return url;
        } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.email) return alert("Email Address is required for Customer Portal Access");
        ...
        try {
            ...
            navigate('/records/customers');

        } catch (error) {
            console.error("Registration failed:", error);
            alert("Failed to register buddy.");
        } finally {
    ...
    return (
        ...
        >
            ...
            <div className="sticky top-0 z-20 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-md px-4 py-3 flex items-center justify-between border-b border-slate-200 dark:border-slate-800">
                ...
                <h1 className="text-lg font-bold">New Buddy</h1>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="bg-white dark:bg-[#1e2736] rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
                        ...
                        <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-slate-700 dark:text-slate-300">Email Address * (Login ID)</label>
                                <input
                                    type="email" name="email" required
                                    value={formData.email} onChange={handleInputChange}
                                    placeholder="Required for Portal Access"
                                    className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-[#1a2230] focus:ring-2 focus:ring-primary outline-none"
                                />
                            </div>
                    ...
                    {/* Photo Upload - CLOUDINARY ENABLED */}
                    <div className="bg-white dark:bg-[#1e2736] rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
                        <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                            <h2 className="font-bold text-base flex items-center gap-2">
                        ...
                        <div className="p-6 flex flex-col sm:flex-row gap-6 items-center">
                            <div className="relative h-32 w-32 rounded-xl bg-slate-100 dark:bg-slate-800 border-2 border-dashed border-slate-300 dark:border-slate-600 flex items-center justify-center overflow-hidden">
                                {photoPreview ? (
                                    ...
                                    <span className="material-symbols-outlined text-center text-xs text-slate-400">
                                        Cloudinary<br />Upload
                                    </span>
                                )}
                            </div>
                            <div className="flex-1 w-full">
                                ...
                                />
                                <p className="text-xs text-slate-400 mt-2">Safe & Compliant Storage via Cloudinary</p>
                            </div>
                        </div>
                    </div>
                    ...
                    >
                        {isSubmitting ? (
                            ...
                            <>Save & Invite <span className="material-symbols-outlined material-symbols-fill">send</span></>
                        )}
                    </button>
                </form>
            </div>
