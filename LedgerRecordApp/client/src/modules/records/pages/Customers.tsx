      ...
      <header className="sticky top-0 z-10 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-sm transition-colors duration-200" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
        <div className="flex items-center justify-between px-4 py-4">
          <div className="flex items-center gap-3">
            <Link to="/records" className="flex h-10 w-10 items-center justify-center rounded-full hover:bg-black/5 dark:hover:bg-white/10 active:scale-95 transition-all">
              <span className="material-symbols-outlined">arrow_back</span>
            </Link>
            <h1 className="text-2xl font-bold tracking-tight">Customers</h1>
          </div>
          <Link to="/records/customers/new" className="flex h-10 w-10 items-center justify-center rounded-full bg-primary text-white shadow-lg shadow-primary/30 hover:bg-primary/90 active:scale-95 transition-all">
      ...
      <main className="flex flex-col gap-3 px-4 pt-2">
        {loading ? (
          ...
          filteredCustomers.map((customer) => (
            <Link
              to={`/records/customers/${customer.id}`}
              key={customer.id}
            ...
            >
              ...
              <div className="flex flex-col items-end gap-2 pl-2">
                ...
                <button
                  onClick={(e) => {
                    e.preventDefault();
                    window.location.href = `/records/customers/edit/${customer.id}`;
                  }}
                  className="flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 text-slate-600 hover:bg-slate-200 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600 transition-colors"
                  title="Edit Details"
                >
