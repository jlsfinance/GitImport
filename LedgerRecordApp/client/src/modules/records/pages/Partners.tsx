...
interface Record { id: string; amount: number; serviceCharge: number; rate: number; tenure: number; installmentAmount: number; activationDate: string; givenAmount: number; repaymentSchedule: { dueDate: string, status: 'Paid' | 'Pending' }[] }
interface Receipt { id: string; recordId: string, amount: number; paymentDate: string; installmentNumber: number; }
...
const Partners: React.FC = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const recordsSnap = await getDocs(query(collection(db, "records"), where("status", "in", ["Approved", "Settled", "Active", "Overdue"])));
            const recordsData = recordsSnap.docs.map(doc => {
                ...
                return {
                    ...
                    activationDate: data.activationDate || data.disbursalDate || '',
                    givenAmount: data.givenAmount || data.disbursedAmount || data.actualDisbursed || data.amount || 0
                };
            }) as Record[];
    ...
    const monthlyCalculations = useMemo(() => {
        if (!selectedMonth) return { totalProfit: 0, processingFees: 0, markupCollected: 0, profitSplits: [] };

        const monthStart = startOfMonth(parseISO(`${selectedMonth}-01`));
        const monthEnd = endOfMonth(parseISO(`${selectedMonth}-01`));

        const processingFees = records
            .filter(l => l.activationDate && isWithinInterval(parseISO(l.activationDate), { start: monthStart, end: monthEnd }))
            .reduce((sum, l) => sum + (Number(l.serviceCharge) || 0), 0);

        // Estimated markup collected (neutral term for service fee component)
        const markupCollected = receipts
            .filter(r => r.paymentDate && isWithinInterval(parseISO(r.paymentDate), { start: monthStart, end: monthEnd }))
            .reduce((sum, receipt) => {
                // LEGACY FIELD SUPPORT: Check both recordId and loanId for backward compat
                const record = records.find(l => l.id === (receipt.recordId || (receipt as any).loanId));
                if (!record) return sum;
                const monthlyRate = (record.rate || 0) / 12 / 100;

                let balance = Number(record.amount);
                const markupComponent = (balance * monthlyRate);
                return sum + markupComponent;
            }, 0);

        // Simplified Total Profit = Fees + Estimated Markup
        const totalProfit = processingFees + markupCollected;

        if (partners.length === 0) return { totalProfit, processingFees, markupCollected, profitSplits: [] };
        ...
        return { totalProfit, processingFees, markupCollected, profitSplits };
    ...
    const monthlyCashFlow = useMemo(() => {
        ...
        const installCollected = receipts
            .filter(r => r.paymentDate && isWithinInterval(parseISO(r.paymentDate), { start: monthStart, end: monthEnd }))
            .reduce((sum, r) => sum + Number(r.amount), 0);

        const recordsActivated = records
            .filter(l => l.activationDate && isWithinInterval(parseISO(l.activationDate), { start: monthStart, end: monthEnd }))
            .reduce((sum, l) => sum + (l.givenAmount || Number(l.amount)), 0);

        const totalInflow = investments + installCollected;
        const totalOutflow = withdrawals + recordsActivated;
        ...
        >
            ...
            <div className="sticky top-0 z-10 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-sm px-4 py-4 border-b border-slate-200 dark:border-slate-800">
                <div className="flex items-center gap-3">
                    <Link to="/records/finance" className="flex h-10 w-10 items-center justify-center rounded-full hover:bg-black/5 dark:hover:bg-white/10 active:scale-95 transition-all">
                    ...
                    <div className="flex flex-col md:flex-row gap-6">
                        ...
                        <div className="md:w-1/3 space-y-4">
                            ...
                            <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl space-y-2 text-sm">
                                ...
                                <div className="flex justify-between"><span>Est. Markup:</span> <span className="font-medium">{formatCurrency(monthlyCalculations.markupCollected)}</span></div>
