        ...
        for (const record of records) {
            ...
            if (['Active', 'Approved', 'Overdue'].includes(record.status) && record.repaymentSchedule) {
                ...
                if (nextInstallment) {
                    ...
                    let body = `Payment of Rs. ${nextInstallment.amount} is due today for ${record.customerName}`;
                    let title = 'Payment Due Today';
                    ...
                    if (isToday(dueDate)) {
                        ...
                        body = `Dear ${record.customerName}, your scheduled payment of ₹${nextInstallment.amount} is due today. Please update your record.`;
                    } else if (isPast(dueDate)) {
                        // It is OVERDUE.
                        title = 'Payment Missed';
                        body = `Dear ${record.customerName}, your payment of ₹${nextInstallment.amount} was scheduled for ${nextInstallment.dueDate}. Please update your record.`;
