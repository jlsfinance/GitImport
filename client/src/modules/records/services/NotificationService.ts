...
import { FinancialRecord as Record } from '../types';
import { parseISO, isToday, setHours, setMinutes, setSeconds, isPast } from 'date-fns';
...
export const NotificationService = {
    ...
    async scheduleRecordNotifications(records: Record[]) {
        ...
        for (const record of records) {
            ...
            const schedule = (record as any).repaymentSchedule || (record as any).paymentSchedule;
            if (['Active', 'Approved', 'Overdue'].includes(record.status) && schedule) {

                // Find the NEXT unpaid installment
                const nextInstallment = schedule.find((inst: any) => inst.status === 'Pending');

                if (nextInstallment) {
                    const dueDate = parseISO(nextInstallment.dueDate);
                    ...
                    let body = `Reminder: ₹${nextInstallment.amount} is scheduled for today. Tap to view ${record.customerName}'s record.`; // Neutral language for compliance
                    let title = 'Record Reminder';
                    ...
                    if (isToday(dueDate)) {
                        ...
                        title = 'Payment Scheduled Today';
                        body = `Friendly reminder: ${record.customerName}'s scheduled payment of ₹${nextInstallment.amount} is due today.`;
                    } else if (isPast(dueDate)) {
                        // It is OVERDUE - use softer language
                        title = 'Record Update Needed';
                        body = `${record.customerName}'s scheduled payment of ₹${nextInstallment.amount} from ${nextInstallment.dueDate} may need attention.`;
