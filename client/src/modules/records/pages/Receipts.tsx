...
interface Receipt {
    id: string;
    recordId: string;
    loanId?: string; // For backward compatibility
    customerId: string;
    customerName: string;
    amount: number;
    paymentDate: string;
    ...
    emiNumber?: number; // For backward compatibility
    companyId?: string;
}
        ...
        try {
            ...
            try {
                ...
                if (customerSnap.exists()) {
                    // Customer data available for future usage if needed
                    console.log("Customer data loaded for receipt generation");
                }
            } catch (e) {
            ...
            y += 7;
            pdfDoc.text(`Record ID: ${receipt.recordId || receipt.loanId}`, 14, y);
            y += 8;
            ...
            y += 8;
            pdfDoc.setFont("helvetica", "normal");
            pdfDoc.text(`Installment Payment (No. ${receipt.installmentNumber || receipt.emiNumber || 'N/A'})`, 14, y);
                                ...
                                {loading ? (
                                    ...
                                    receipts.map((receipt) => (
                                        <tr key={receipt.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                            ...
                                            <td className="px-4 py-3 text-slate-500 font-mono text-xs">{receipt.recordId || receipt.loanId}</td>
