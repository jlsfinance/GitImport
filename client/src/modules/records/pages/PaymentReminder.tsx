...
import { format, parseISO, differenceInDays, startOfDay, isPast } from 'date-fns';
...
interface Record {
    ...
    customerId: string;
    date?: string;
    // LEGACY FIELD: emiNumber kept for backward compatibility, use installmentNumber
    repaymentSchedule: { emiNumber?: number; installmentNumber?: number; amount: number; dueDate: string; status: 'Pending' | 'Paid' | 'Cancelled' }[];
}
...
const PaymentReminder: React.FC = () => {
    ...
    const [form, setForm] = useState<NoticeForm>({
        ...
        overdueChargePerDay: 50,
        paymentDetails: `UPI ID: ${SUPPORT_PHONE}-3@ybl\nUse UPI for fast settlement.`,
        signatoryName: 'Administration',
        customDueDate: ''
    });

    // Fetch Smart Data (Customers with Overdue Installments)
    const fetchSmartData = useCallback(async () => {
        ...
        setLoadingData(true);
        try {
            const [customersSnap, recordsSnap] = await Promise.all([
                getDocs(query(
                    collection(db, "customers"),
                    where("companyId", "==", currentCompany.id)
                )),
                getDocs(query(
                    collection(db, "records"), // Updated from 'loans' for neutrality
            ...
            const allRecords = recordsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() })) as Record[];
            ...
            allRecords.forEach(record => {
                if (record.repaymentSchedule) {
                    const hasOverdue = record.repaymentSchedule.some(inst =>
                        inst.status === 'Pending' && isPast(parseISO(inst.dueDate))
                    );
    ...
    const handleCustomerSelect = (customer: Customer) => {
        ...
        const customerRecords = records.filter(l => l.customerId === customer.id);

        // Find the oldest overdue Installment
        let targetRecord: Record | undefined;
        let targetInst: any;

        for (const record of customerRecords) {
            if (!record.repaymentSchedule) continue;
            const overdueInst = record.repaymentSchedule.find(inst =>
                inst.status === 'Pending' && isPast(parseISO(inst.dueDate))
            );
            if (overdueInst) {
                targetRecord = record;
                targetInst = overdueInst;
                break;
            }
        }

        if (targetRecord && targetInst) {
            setForm(prev => ({
                ...prev,
                customerName: customer.name,
                customerAddress: customer.address || '',
                recordId: targetRecord!.id,
                // LEGACY: Support both installmentNumber and emiNumber
                installmentNumber: targetInst.installmentNumber || targetInst.emiNumber || 1,
                installmentAmount: targetInst.amount,
                startDate: targetRecord!.date || '',
                customDueDate: targetInst.dueDate,
                issueDate: format(new Date(), 'yyyy-MM-dd')
            }));
        } else {
            setForm(prev => ({
                ...
                customerName: customer.name,
                customerAddress: customer.address || '',
                recordId: customerRecords[0]?.id || '',
                startDate: customerRecords[0]?.date || '',
                issueDate: format(new Date(), 'yyyy-MM-dd')
            }));
        }
    };
    ...
    const totalAmountDue = useMemo(() => {
        const lateFee = daysOverdue * form.overdueChargePerDay;
        return form.installmentAmount + lateFee;
    }, [form.installmentAmount, daysOverdue, form.overdueChargePerDay]);

    const fullReminderText = useMemo(() => {
        return `To:
Mr./Ms. ${form.customerName}
${form.customerAddress}
Record ID: ${form.recordId}

Subject: REMINDER for Due Payment

This value-added message is a friendly reminder regarding your pending payment with ${companyDetails.name}.

Your Installment (No. ${form.installmentNumber}), which was due on ${calculatedDueDate ? format(parseISO(calculatedDueDate), 'dd MMMM, yyyy') : '---'}, is pending. The payment is overdue by ${daysOverdue} days.

DETAILS:
- Installment Amount: ${formatCurrency(form.installmentAmount)}
- Overdue Charges (${daysOverdue} days): ${formatCurrency(daysOverdue * form.overdueChargePerDay)}
- TOTAL OUTSTANDING: ${formatCurrency(totalAmountDue)}

Please arrange to make the payment of ${formatCurrency(totalAmountDue)} within ${form.paymentDeadlineHours} hours.
...
For ${companyDetails.name}`;
    }, [form, companyDetails, calculatedDueDate, daysOverdue, totalAmountDue]);
    ...
    const generatePdf = () => {
        ...
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(16);
        doc.text("RECORD UPDATE NOTICE", pageWidth / 2, y, { align: 'center' });
        y += 15;
        ...
        // Content
        doc.setFontSize(11);
        const splitText = doc.splitTextToSize(fullReminderText, pageWidth - (margin * 2));
    ...
    const saveTemplate = () => {
        ...
        localStorage.setItem('reminderTemplates', JSON.stringify(updated));
        setNewTemplateName('');
    ...
    if (!selectedCustomer) {
        return (
            <div className="min-h-screen bg-background-light dark:bg-background-dark pb-24 text-slate-900 dark:text-white">
                {/* Disclaimer */}
                <div className="bg-yellow-100 dark:bg-yellow-900/30 p-1 text-center text-[10px] font-bold text-yellow-800 dark:text-yellow-400">
                    RECORD KEEPING ONLY - NOT A DEBT COLLECTION SERVICE
                </div>
                <div className="sticky top-0 z-10 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-sm px-4 py-4 border-b border-slate-200 dark:border-slate-800">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-2xl font-bold tracking-tight">Payment Reminder</h1>
                    </div>
                </div>
                ...
                <div className="max-w-xl mx-auto p-4 space-y-6">
                    <div className="bg-white dark:bg-[#1e2736] rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
                        <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                            ...
                            <p className="text-xs text-slate-500">Only showing customers with overdue payments</p>
                        </div>
                        ...
                        <div className="p-4">
                            ...
                            {loadingData ? (
                                ...
                                <div className="text-center py-8 text-slate-400">
                                    ...
                                    <p>No customers with overdue payments found!</p>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            <div className="max-w-6xl mx-auto p-4 grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-6">
                    <div className="bg-white dark:bg-[#1e2736] rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-5 space-y-4">
                        <h2 className="font-bold text-lg border-b border-slate-100 dark:border-slate-800 pb-2">Details</h2>

                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="text-xs font-bold text-slate-500">Record ID</label>
                                <input type="text" value={form.recordId} onChange={e => setForm({ ...form, recordId: e.target.value })} className="w-full mt-1 p-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-sm" />
                            </div>
                            <div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="text-xs font-bold text-slate-500">Amount Due</label>
                                <input type="number" value={form.installmentAmount} onChange={e => setForm({ ...form, installmentAmount: Number(e.target.value) })} className="w-full mt-1 p-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-sm" />
                            </div>
                            <div>
                        ...
                        <div className="p-3 bg-red-50 dark:bg-red-900/10 rounded-xl border border-red-100 dark:border-red-900/30 flex justify-between items-center text-sm">
                            ...
                            <div className="text-right">
                                <span className="block text-xs text-red-600">Total Outstanding</span>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="text-xs font-bold text-slate-500">Overdue Charge / Day</label>
                                <input type="number" value={form.overdueChargePerDay} onChange={e => setForm({ ...form, overdueChargePerDay: Number(e.target.value) })} className="w-full mt-1 p-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-sm" />
                            </div>
                        </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-green-50 dark:bg-green-900/10 rounded-2xl border border-green-100 dark:border-green-900/30 p-5">
                        ...
                        <div className="bg-white dark:bg-[#151b26] p-3 rounded-lg text-xs border border-green-100 dark:border-green-900/20 text-slate-600 dark:text-slate-300">
                            🔔 *RECORD UPDATE NOTICE* 🔔<br /><br />
                            Dear {form.customerName},<br />
                            Payment of *{formatCurrency(totalAmountDue)}* due on {calculatedDueDate} is pending.<br />
                            Please update your record within {form.paymentDeadlineHours} hours.<br /><br />
                            UPI: {form.paymentDetails.split('\n')[0].split(':')[1] || 'As per notice'}
                        </div>
                        <button
                            onClick={() => {
                                const msg = `🔔 *RECORD UPDATE NOTICE* 🔔\n\nDear ${form.customerName},\nYour record of *${formatCurrency(totalAmountDue)}* (due on ${calculatedDueDate}) may need attention.\nPlease update within ${form.paymentDeadlineHours} hours.\n\nUPI: ${form.paymentDetails.split('\n')[0].split(':')[1] || 'As per notice'}`;
                                const phone = selectedCustomer.phone?.replace(/\D/g, '').slice(-10);
                                if (phone) {
                                    const confirmSend = window.confirm(`Open WhatsApp to send reminder to ${form.customerName}?`);
                                    if (confirmSend) {
