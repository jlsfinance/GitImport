...
import { recordsAuth as auth } from '../../../lib/firebase';
...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { code } = handleError('AUTH_LOGIN_FAILED', err);
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const { code } = handleError('AUTH_LOGIN_FAILED', err);
      setError(`Google Sign-In failed [${code}]. Check configuration.`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      {/* Module Switcher Button - HIDDEN for Standalone App
      <button
        onClick={() => {
          localStorage.removeItem('selected_module');
          navigate('/');
        }}
        className="absolute left-4 z-50 flex items-center gap-2 px-3 py-2 bg-white/10 hover:bg-white/20 backdrop-blur-sm rounded-full border border-white/20 transition-all active:scale-95"
        style={{ top: 'calc(env(safe-area-inset-top) + 16px)' }}
      >
        <span className="material-symbols-outlined text-[18px]">swap_horiz</span>
        <span className="text-[10px] font-bold uppercase tracking-wider">Switch Module</span>
      </button> 
      */}
      ...
      <div className="w-full max-w-sm flex flex-col items-center z-10 h-full justify-evenly">
        ...
        <form onSubmit={handleLogin} className="w-full space-y-3">
          ...
          <div>
            ...
            <div className="flex justify-end mt-1">
              <Link to="/records/forgot-password" className="text-[10px] text-white/60 hover:text-white transition-colors">Forgot password?</Link>
            </div>
          </div>
          ...
          <div className="flex items-start gap-2 mt-1">
            ...
            <label htmlFor="terms" className="text-[10px] text-white/70 leading-tight">
              I agree to the <Link to="/records/terms" className="text-white font-bold hover:underline">Terms of Use</Link>, <Link to="/records/privacy" className="text-white font-bold hover:underline">Privacy Policy</Link>, <Link to="/records/cookie-policy" className="text-white font-bold hover:underline">Cookie Policy</Link>, and all <Link to="/records/legal" className="text-white font-bold hover:underline">Legal Documents</Link>.
            </label>
          </div>
        ...
        <div className="w-full space-y-4">
          ...
          {/* TEMPORARILY DISABLED: Customer Portal
          <Link to="/records/customer-login" className="w-full border border-white/20 bg-white/5 py-3 rounded-full flex items-center justify-center gap-2 active:scale-95 transition-all text-xs font-bold hover:bg-white/10">
