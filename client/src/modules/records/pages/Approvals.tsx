...
const Approvals: React.FC = () => {
    ...
    const [applications, setApplications] = useState<RecordEntry[]>([]);
    const [loading, setLoading] = useState(true);

    // Modal State
    const [modalType, setModalType] = useState<'accept' | 'reject' | null>(null);
    const [selectedApp, setSelectedApp] = useState<RecordEntry | null>(null);
    ...
    useEffect(() => {
        const fetchPendingEntries = async () => {
            ...
            setLoading(true);
            try {
                // Fetch 'Draft' records (Neutral Terminology)
                // Use 'in' operator to catch legacy 'Pending' as well as new 'Draft'
                const q = query(
                ...
                const pendingEntries = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as RecordEntry[];
                setApplications(pendingEntries);
            } catch (error) {
    ...
    const openModal = (type: 'accept' | 'reject', app: RecordEntry) => {
        setModalType(type);
        setSelectedApp(app);
        setComment('');
    };
    ...
    const handleStatusUpdate = async () => {
        ...
        // Neutral Statuses: 'Active' (Verified) or 'Voided' (Discarded)
        // 'Approved' is strictly forbidden.
        const newStatus = modalType === 'accept' ? 'Active' : 'Voided';

        try {
            const recordRef = doc(db, "records", selectedApp.id);

            const updateData: any = {
                status: newStatus,
                verificationNote: comment, // Neutral term
            };
            ...
            alert(`Entry ${newStatus === 'Active' ? 'Verified' : 'Discarded'} Successfully`);
            closeModal();
    ...
    return (
        ...
        >
            <div className="flex items-center justify-between mb-6">
                <h1 className="text-xl font-bold">Verify Entries</h1>
                <span className="text-xs bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full">Drafts: {applications.length}</span>
            </div>
            {/* ... rest of the UI needs update to match new terminology ... */}
            {/* Header */}
            <div className="sticky top-0 z-10 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-sm px-4 py-4 border-b border-slate-200 dark:border-slate-800">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-2xl font-bold tracking-tight">Verify Entries</h1>
                </div>
            </div>
            ...
            <div className="max-w-4xl mx-auto p-4 space-y-6">
                <div className="bg-white dark:bg-[#1e2736] rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
                    <div className="p-4 border-b border-slate-100 dark:border-slate-800">
                        <h2 className="font-bold text-lg">Draft Entries</h2>
                        <p className="text-sm text-slate-500">Review and process new record entries.</p>
                    </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
                    <div className="bg-white dark:bg-[#1e2736] rounded-2xl w-full max-w-sm shadow-2xl p-6">
                        <h3 className="text-lg font-bold mb-1">
                            {modalType === 'accept' ? 'Approve Record?' : 'Reject Record?'}
                        </h3>
                        <p className="text-sm text-slate-500 mb-4">
                            {modalType === 'accept'
                                ? `You are approving a record of Rs. ${selectedApp.amount.toLocaleString('en-IN')} for ${selectedApp.customerName}.`
                                : `You are rejecting the entry for ${selectedApp.customerName}.`}
                        </p>
                        ...
                        <div className="flex gap-3 justify-end">
                            ...
                            >
                                ...
                                Confirm {modalType === 'accept' ? 'Approval' : 'Rejection'}
                            </button>
                        </div>
                    </div>
                </div>
