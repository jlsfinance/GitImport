...
const DueList: React.FC = () => {
    ...
    const fetchPendingInstallments = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const validStatuses = ["Active", "Overdue", "Disbursed", "Given", "Pending"];

            const [recordsSnapshot, loansSnapshot, customersSnapshot] = await Promise.all([
                getDocs(query(collection(db, "records"), where("status", "in", validStatuses), where("companyId", "==", currentCompany.id))),
                getDocs(query(collection(db, "loans"), where("status", "in", validStatuses), where("companyId", "==", currentCompany.id))),
            ...
            // Process BOTH Records and Loans
            const allDocs = [...recordsSnapshot.docs, ...loansSnapshot.docs];
            ...
            console.error("Failed to load pending installments:", error);
        } finally {
            setLoading(false);
        }
    }, [currentCompany]);
    ...
    const handleBulkSendReminders = () => {
        if (filteredInstallments.length === 0) return alert("No installments to remind.");

        let count = 0;
        filteredInstallments.forEach((inst, index) => {
            if (inst.phoneNumber && inst.phoneNumber.length >= 10) {
                setTimeout(() => handleSendReminder(inst), index * 500);
                count++;
            }
        });
    ...
    const handleDownloadReport = async () => {
        try {
            ...
            doc.text(`Total Due: ${formatCurrency(totalDueValue)}`, 14, 40);
