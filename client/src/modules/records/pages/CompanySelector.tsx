...
const CompanySelector: React.FC = () => {
  ...
  const { companies, currentCompany, setCurrentCompany, addCompany, deleteCompany, updateCompany, loading } = useCompany();
  ...
  useEffect(() => {
    const checkOrphanedData = async () => {
      try {
        const [allCustomers, allRecords, allPartners, allExpenses] = await Promise.all([
        ...
        const orphanRecords = allRecords.docs.filter(d => !d.data().companyId).length;
  ...
  const handleMigrateData = async () => {
    ...
    try {
      ...
      const [customersSnap, recordsSnap, partnersSnap, expensesSnap] = await Promise.all([
        getDocs(collection(db, "customers")),
        getDocs(collection(db, "records")),
        getDocs(collection(db, "partner_transactions")),
        getDocs(collection(db, "expenses"))
