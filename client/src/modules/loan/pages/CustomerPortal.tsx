...
const CustomerPortal: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const fetchedLoans = lSnap.docs.map(d => {
        const data = d.data();
        // Map repaymentSchedule to ensure it has 'date' property (mapped from dueDate if missing)
        const schedule = data.repaymentSchedule?.map((emi: any) => ({
          ...emi,
          date: emi.date || emi.dueDate, // Ensure date exists for shared type compatibility
      ...
      setLoans(fetchedLoans.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
                ...
                {[
                  ...
                  },
                  {
                    ...
                    action: async () => {
                      if (!customer || !company) return alert("Please wait, data is still loading...");
                      try {
                        const closedLoan = loans.find(l => (l.status as string) === 'Closed' || (l.status as string) === 'Paid' || l.status === 'Completed');
                        if (closedLoan) {
