...
const LOAN_TERMS = [
    ...
    "2. Default in payment will attract late fees as per company policy.",
    "3. The credit is recorded against the documents provided (if any).",
    "4. The company reserves the right to recover the balance in case of default.",
...
export class PdfGenerator {
    ...
    static async generateLoanAgreement(loan: Loan, customer: Customer, company: Company) {
        ...
        pdf.setFontSize(16);
        pdf.setTextColor(0);
        pdf.text("SERVICE AGREEMENT", 105, 45, { align: "center" });
        ...
        pdf.text(`Record Amount: ${formatCurrency(loan.amount)}`, 20, 75);
        ...
        await savePdf(pdf, `Service_Agreement_${loan.id}.pdf`);
    }
    ...
    static async generateLoanCard(loan: Loan, customer: Customer, company: Company) {
        ...
        await savePdf(pdf, `RecordCard_${loan.id}.pdf`);
    }
    ...
    static async generateAccountStatement(loans: Loan[], customer: Customer, company: Company) {
        ...
        autoTable(pdf, {
            startY: 70,
            head: [['Date', 'Transaction Type', 'Reference', 'Debit (Given)', 'Credit (Payment)']],
            body: tableData,
            theme: 'striped',
    ...
    static async generateNoDuesCertificate(loan: Loan, customer: Customer, company: Company) {
        ...
        const certificateText = `This is to certify that Mr./Ms. ${customer.name} having Record Account Number ${loan.id} has successfully settled the entire record amount along with all applicable charges.

As on ${format(new Date(), 'dd-MMM-yyyy')}, there are no outstanding dues against the aforementioned record account.

The company has no further claim or lien on this account. This certificate is issued at the request of the customer for their records.`;
        ...
        pdf.setFont("helvetica", "normal");
        contentY += 10;
        pdf.text(`Total Amount: ${formatCurrency(loan.amount)}`, 35, contentY);
        contentY += 7;
