          ...
          >
            <div className="max-h-[320px] overflow-y-auto space-y-1 no-scrollbar">
              ...
              {type === 'customer' && !hasContactPerm && query.length >= 2 && (
                <div className="mt-2 pt-2 border-t border-border/50">
                  <p className="px-5 text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-3">Phone Contacts</p>
                  <button
                    type="button"
                    onClick={() => setShowPermissionModal(true)}
                    className="w-full text-left p-4 rounded-[24px] flex items-center gap-4 text-google-blue bg-google-blue/10 hover:bg-google-blue/20 transition-all group/permission border border-google-blue/10"
                  >
                    <div className="w-12 h-12 rounded-full bg-google-blue/20 flex items-center justify-center group-hover/permission:scale-110 transition-transform">
                      <Search className="w-6 h-6" />
                    </div>
                    <div>
                      <div className="font-bold text-xs uppercase tracking-widest opacity-70">Search Phone</div>
                      <div className="text-sm font-black text-foreground">Tap to find "{query}"</div>
                    </div>
                  </button>
                </div>
              )}
