...
import { Plus, Trash2, Save, X, CreditCard, Banknote, ArrowLeft, AlertCircle, Calculator, Package, UserPlus, FileText, RotateCcw } from 'lucide-react';
...
const CreateInvoice: React.FC<CreateInvoiceProps> = ({ onSave, onCancel, initialInvoice, startSmartCalc, initialCustomerId }) => {
  ...
  const [invoiceType, setInvoiceType] = useState<'SALE' | 'SALES_RETURN'>('SALE');
  ...
  useEffect(() => {
    ...
    if (initialInvoice) {
      ...
      setInvoiceType((initialInvoice.type as any) === 'SALES_RETURN' ? 'SALES_RETURN' : 'SALE');
  ...
  return (
    <div className="bg-background min-h-screen pb-48 font-sans">
      ...
      <div className="max-w-5xl mx-auto space-y-3 pt-3 px-3">
        ...
        <div className="bg-surface-container p-1 rounded-full border border-border flex items-center mb-1">
          <button
            onClick={() => { setInvoiceType('SALE'); setHasChanges(true); }}
            className={`flex-1 py-3 px-4 rounded-full flex items-center justify-center gap-2 transition-all text-sm font-bold ${invoiceType === 'SALE'
              ? 'bg-google-blue text-white shadow-md'
              : 'text-muted-foreground hover:bg-surface-container-high'
              }`}
          >
            <FileText className="w-4 h-4" /> <span>Sale Bill</span>
          </button>
          <button
            onClick={() => { setInvoiceType('SALES_RETURN'); setHasChanges(true); }}
            className={`flex-1 py-3 px-4 rounded-full flex items-center justify-center gap-2 transition-all text-sm font-bold ${invoiceType === 'SALES_RETURN'
              ? 'bg-amber-600 text-white shadow-md'
              : 'text-muted-foreground hover:bg-surface-container-high'
              }`}
          >
            <RotateCcw className="w-4 h-4" /> <span>Credit Note</span>
          </button>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-[24px] shadow-sm border border-slate-100 dark:border-slate-700">
          <div className="flex items-center justify-between mb-3">
            <label className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
              <span className="p-1 bg-purple-100 text-purple-600 rounded">DISCOUNT</span>
            </label>
            <div className="flex bg-slate-100 dark:bg-slate-700 p-1 rounded-lg">
              <button
                onClick={() => setDiscountType('PERCENTAGE')}
                className={`px-4 py-1.5 rounded-md text-[10px] font-black transition-all ${discountType === 'PERCENTAGE' ? 'bg-white dark:bg-slate-600 shadow-sm text-foreground' : 'text-slate-400'}`}
              >
                %
              </button>
              <button
                onClick={() => setDiscountType('AMOUNT')}
                className={`px-4 py-1.5 rounded-md text-[10px] font-black transition-all ${discountType === 'AMOUNT' ? 'bg-white dark:bg-slate-600 shadow-sm text-foreground' : 'text-slate-400'}`}
              >
                ₹
              </button>
            </div>
          ...
          <div className="relative">
            <input
              type="number"
              value={discountValue || ''}
              onChange={(e) => { setDiscountValue(Number(e.target.value)); setHasChanges(true); }}
              className="w-full bg-slate-50 dark:bg-slate-900 border-none rounded-xl p-4 text-xl font-black text-foreground outline-none focus:ring-2 focus:ring-purple-500/20"
              placeholder={discountType === 'PERCENTAGE' ? "0%" : "₹0"}
            />
            {discountValue > 0 && discountType === 'PERCENTAGE' && (
              <div className="absolute right-4 top-1/2 -translate-y-1/2 px-3 py-1 bg-green-100 text-green-700 rounded-lg text-xs font-bold">
                -₹{Math.round((calculateSubtotal() * discountValue) / 100).toLocaleString()}
              </div>
            )}
          </div>
        </div>
