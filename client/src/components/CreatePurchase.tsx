...
import { Save, ArrowLeft, Package, Users, ShoppingCart, RotateCcw, Banknote } from 'lucide-react';
...
const CreatePurchase: React.FC<CreatePurchaseProps> = ({ onSave, onCancel, initialPurchase }) => {
    ...
    useEffect(() => {
        ...
        if (initialPurchase) {
            ...
            setInvoiceType((initialPurchase.type as any) === 'PURCHASE_RETURN' ? 'PURCHASE_RETURN' : 'PURCHASE');
    ...
    const calculateTaxes = (item: InvoiceItem) => {
        ...
        <div className="bg-background min-h-screen pb-48 font-sans">
            ...
            <div className="max-w-5xl mx-auto space-y-4 pt-6 px-4">
                ...
                <div className="bg-surface-container p-1 rounded-full border border-border flex items-center">
                    <button
                        onClick={() => { setInvoiceType('PURCHASE'); setHasChanges(true); }}
                        className={`flex-1 py-3 px-4 rounded-full flex items-center justify-center gap-2 transition-all text-sm font-bold ${invoiceType === 'PURCHASE'
                            ? 'bg-orange-600 text-white shadow-md'
                            : 'text-muted-foreground hover:bg-surface-container-high'
                            }`}
                    >
                        <ShoppingCart className="w-4 h-4" /> <span>Purchase Bill</span>
                    </button>
                    <button
                        onClick={() => { setInvoiceType('PURCHASE_RETURN'); setHasChanges(true); }}
                        className={`flex-1 py-3 px-4 rounded-full flex items-center justify-center gap-2 transition-all text-sm font-bold ${invoiceType === 'PURCHASE_RETURN'
                            ? 'bg-red-600 text-white shadow-md'
                            : 'text-muted-foreground hover:bg-surface-container-high'
                            }`}
                    >
                        <RotateCcw className="w-4 h-4" /> <span>Debit Note</span>
                    </button>
                </div>
