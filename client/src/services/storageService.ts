...
export const StorageService = {
  ...
  savePurchase: (purchase: Invoice) => {
    ...
    if (cIndex >= 0 && purchase.status === 'PENDING') {
      ...
      if (isReturn) {
        vendor.balance += purchase.total; // We owe less
      } else {
        vendor.balance -= purchase.total; // We owe more
      }
  ...
  updatePurchase: (updatedPurchase: Invoice) => {
    ...
    // 4. Update Purchase: Complex Revert + Apply.
    // Simplifying: If type changed, or quantities changed. 
    // Handled by calling deletePurchase then savePurchase? No, ID must persist.
    // Revert Old
    if (oldPurchase.type === 'PURCHASE_RETURN') {
      // Revert Return (Stock +, Balance -)
      oldPurchase.items.forEach(item => {
        const p = cache.products.find(prod => prod.id === item.productId);
        if (p) p.stock += item.quantity;
      });
      const c = cache.customers.find(cust => cust.id === oldPurchase.customerId);
      if (c) c.balance -= oldPurchase.total;
    } else {
      // Revert Purchase (Stock -, Balance +)
      oldPurchase.items.forEach(item => {
        const p = cache.products.find(prod => prod.id === item.productId);
        if (p) p.stock -= item.quantity;
      });
    ...
    // Apply New
    if (updatedPurchase.type === 'PURCHASE_RETURN') {
      // Apply Return (Stock -, Balance +)
      updatedPurchase.items.forEach(item => {
        const p = cache.products.find(prod => prod.id === item.productId);
        if (p) p.stock -= item.quantity;
        if (item.rate > 0 && p) p.purchasePrice = item.rate;
      });
      const c = cache.customers.find(cust => cust.id === updatedPurchase.customerId);
      if (c) c.balance += updatedPurchase.total;
    } else {
      // Apply Purchase (Stock +, Balance -)
      updatedPurchase.items.forEach(item => {
        const p = cache.products.find(prod => prod.id === item.productId);
        if (p) p.stock += item.quantity;
        if (item.rate > 0 && p) p.purchasePrice = item.rate;
  ...
  updateInvoice: (updatedInvoice: Invoice) => {
    ...
    if (oldCIndex >= 0 && oldInvoice.status === 'PENDING') {
      if (oldInvoice.type === 'SALES_RETURN') {
    ...
    if (newCIndex >= 0 && updatedInvoice.status === 'PENDING') {
      if (updatedInvoice.type === 'SALES_RETURN') {
